...
-- PointHistory 데이터 생성
-- PointHistory 데이터 50개 생성
INSERT INTO point_history (amount, changed_at, comment, member_id, types)
SELECT
    FLOOR(RAND() * 500 + 100), -- 랜덤 포인트 금액
    NOW() - INTERVAL FLOOR(RAND() * 100) DAY, -- 최근 100일 내 변경 시간
    CONCAT('Comment ', FLOOR(RAND() * 100)), -- 랜덤 코멘트
    1,
    CASE FLOOR(RAND() * 2) WHEN 0 THEN 'EARN' ELSE 'SPEND' END -- 'EARN' 또는 'SPEND'
FROM
    (SELECT 1 UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10) t
LIMIT 50;

-- OrderGroupPointHistory 데이터 생성
INSERT INTO order_group_point_history (order_group_id, point_history_id)
SELECT
    og.id AS order_group_id, -- OrderGroup ID
    ph.id AS point_history_id -- PointHistory ID
FROM
    order_group og
        JOIN
    point_history ph
    ON ph.member_id = og.user_id -- 회원 ID 기준으로 매칭
WHERE
    ph.types IN ('EARN', 'SPEND') -- 유효한 타입만 사용
  AND RAND() < 0.7 -- 랜덤하게 일부 데이터 선택 (70% 확률)
LIMIT 30;